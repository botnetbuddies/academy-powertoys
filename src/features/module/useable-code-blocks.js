registerFeature({
  ...
  run() {
    ...
    function copyText(text, htmlText) {
      ...
      if (navigator.clipboard && window.isSecureContext) {
        const canWriteRichClipboard =
          typeof navigator.clipboard.write === "function" && typeof ClipboardItem === "function" && typeof Blob === "function" && htmlText;
      ...
      return fallbackCopy(text) ? Promise.resolve() : Promise.reject(new Error("clipboard-unavailable"));
    }
    ...
    function getTerminalKind(language) {
      const lang = String(language || "").toLowerCase();
      const psLanguages = new Set(["powershell", "powershell-session", "powershellsession", "powershell_session", "pwsh", "ps1", "ps"]);
